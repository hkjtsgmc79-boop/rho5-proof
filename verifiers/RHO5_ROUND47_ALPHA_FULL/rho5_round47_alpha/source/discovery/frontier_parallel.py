...
def prepare_wave(work,wave,inputs,target):
    ...
    # Scheduling only: all candidates still pass the unchanged C++ alpha
...
def job_chunk(job,work,seconds):
    ...
    with (d/(stamp+'_DISCOVERY.log')).open('w') as log:
        try:
            subprocess.run([str(model/'discover_focus'),str(d/'checkpoint.tree'),str(d/'next.tree'),str(seconds),str(1 if job['rounds']==1 else 1000000),job['focus']],
    ...
    info.update(target_open=inside,model_sha256=job['model_sha256'],tree_sha256=digest(d/'next.tree'),
                search_seconds=seconds,search_max_solves=1 if job['rounds']==1 else 1000000,focus=job['focus'],rounds=job['rounds'],scope='ONE_FRONTIER_ONLY_NO_WHOLE_ROOT_CREDIT')
