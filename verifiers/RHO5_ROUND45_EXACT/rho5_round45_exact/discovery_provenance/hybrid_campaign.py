...
def main():
 ...
 receipt=json.loads((root/'hybrid_proposal'/a.model/'HYBRID_PROPOSAL_RECEIPT.json').read_text())
 assert fp.digest(model/'model.json')==fp.PINS[a.model]
 proposal=root/'hybrid_proposal'/a.model
 assert fp.digest(proposal/'discover_hybrid.cpp')==receipt['source_sha256']
 assert fp.digest(proposal/'proposal_simplex.hpp')==receipt['precise_simplex_sha256']
 for n,pin in receipt['exact_sources'].items():assert fp.digest(model/n)==pin
 assert fp.digest(proposal/'fast_simplex.hpp')==receipt['fast_simplex_sha256']
 exe=proposal/'discover_hybrid'
 def robust_chunk(job,work,seconds):
  ...
  info.update(target_open=inside,model_sha256=job['model_sha256'],tree_sha256=fp.digest(d/'next.tree'),search_seconds=seconds,
              focus=job['focus'],rounds=job['rounds'],scope='ONE_FRONTIER_ONLY_NO_WHOLE_ROOT_CREDIT',proposal='validated_fast_with_precise_fallback_v1')
 ...
 while inputs and time.monotonic()<deadline:
  ...
  fp.write_json(current,{'status':'ORIGINAL_MODEL_ROOT_COMPLETE' if accepted[a.model]['open']==0 else 'EXACT_PARTIAL_ORIGINAL_ROOT',
    'models':accepted,'last_wave':index,'workers_cap':a.workers,'discovery_proposal':'validated_fast_with_precise_fallback_v1','whole_height_claim_requires_both_complete':True})
