...
本页基于 D158 已接收清单。当前产品入口为 `Rho5.PhaseOne`，实际默认加载634个项目模块；包内646个规范源码文件另含12个非默认模块。完整公共类型以 [P01与示例编译器输出](../FINAL_THEOREM_TYPES.txt) 为准。这里的“无条件”指没有尚未证明的全域安全或存在性责任；各定理的普通域条件、矩阵/路径资格仍以公开类型为准。

Physical X 到实际矩阵的构造方向，与原最大者到 X/B 的来源归约分别登记，不能据前者声称所有矩阵与 Physical X 双向等价。D100/D108是文档索引的可选既有成果，不属于本包默认源码集合。

下列原清单中的历史仓库路径用于溯源；包内源码位于 `../Rho5/`，证据位于 `../evidence/`。
...
* 接收：D09 独立回执 `../evidence/D158/evidence/D09_COORDINATOR_RECEIPT.json`；当前 C02 闭包再次实际消费。
...
* 未含：候选存在已不再是外部输入。
* 接收：`../evidence/D158/evidence/D34_INTAKE_RECEIPT.json`、`../evidence/D158/evidence/D30_ORIGINAL_READY.json`、`../evidence/D158/evidence/D30_ALPHA_READY.json`。
...
* 接收：`../evidence/D158/evidence/D82_INTAKE_RECEIPT.json`（`ACCEPTED_DECLARED_SCOPE`）。
...
* 接收：`../evidence/D158/evidence/D92_GLOBAL_RANGE_READY.json`、`../evidence/D158/evidence/D92_GLOBAL_ATTAINED_READY.json`、C02 回执。
...
* 接收：`../evidence/D158/evidence/D124_INTAKE_RECEIPT.json`；`../evidence/D158/evidence/D86_INTAKE_RECEIPT.json` 为其已验上游（完整第四 pivot）。
...
* 接收：`../evidence/D158/evidence/D103_INTAKE_RECEIPT.json`（`ACCEPTED_A_AND_B_FULL_SCOPE`）；已含在 C02 最终绑定。
...
* 接收：`../evidence/D158/evidence/D131_INTAKE_RECEIPT.json`、`../evidence/D158/evidence/D126_INTAKE_RECEIPT.json`。
...
* 接收：`../evidence/D158/evidence/D141_INTAKE_RECEIPT.json`、`../evidence/D158/evidence/D142_INTAKE_RECEIPT.json`、`../evidence/D158/evidence/D139_INTAKE_RECEIPT.json`、C02 回执。
...
* 完整公开类型：`../evidence/D158/evidence/C02_FULL_TARGET_TYPES.txt`（编译器实际输出，本卡不重新 print 全库）。
* 接收：`../evidence/D158/evidence/C02_INTAKE_RECEIPT.json`（`ACCEPTED_SCOPED`，三门接收）、`../evidence/D158/evidence/C02_ASSEMBLY_*_READY.json`。
...
* 接收：`../evidence/D158/evidence/D84_INTAKE_RECEIPT.json`（`ACCEPTED_DECLARED_SCOPE`）。本阶段作为“有限新增”并入默认主线。

### B3. 可选（**不进入默认闭包**）— Optional, not in the default closure
* `Rho5.Shared.V43PositiveResourceEscape`（命名空间 `Rho5.V43PositiveResourceEscape`）：`exists_escape_with_small_displacement`、`not_isLocalHeightMaxOnPhysical`、`consumed_eq_zero_of_isLocalHeightMaxOnPhysical`；保留局部图/内部资格，不是全 X 覆盖。接收：`../evidence/D158/evidence/D100_INTAKE_RECEIPT.json`。
* `Rho5.Shared.V43CompactBoundary`（命名空间 `Rho5.V43CompactBoundary`）：`isCompact_K`、`exists_height_maxOn`、`maximizer_boundary_or_exhausted`；仍需其 `K` 的源成员/非空资格。接收：`../evidence/D158/evidence/D108_INTAKE_RECEIPT.json`。
...
| `../evidence/D158/evidence/` | 少量核心原回执与完整公开类型（编译器实际输出）副本 |
