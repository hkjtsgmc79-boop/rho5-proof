...
from native_probe import structure,identity
ROOT=Path(__file__).resolve().parent
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def check_chunk(payload):
    backend,items=payload
    if backend=='fraction':from high_value_reference import verify_leaf
    else:from native_backend import verify_leaf
    counts=Counter()
    for box,node in items:
        status=verify_leaf(box,node)
        if status not in ('EMPTY','SAFE'):raise ValueError('unaccepted leaf')
        counts[status]+=1;counts['checked']+=1
        counts['enclosure:'+node.get('enclosure','original_v37')]+=1
...
def replay(tree,workers=40,backend='native',allow_open=False):
    ...
    tp.verify_partition();data=json.loads(tree.read_text());box,route=structure(data,None)
    if route!='':raise ValueError('original whole-root required')
    ...
    assert terminals['checked']==counts['E']+counts['C']+counts['H']
    ...
    result={'status':'COMPLETE_ALPHA_COVERAGE'if not counts['O']else'PARTIAL_EXACT_COVERAGE_ONLY',
            ...
            'tree_sha256':tree_hash,'model_sha256':data['model_sha256'],'source_manifest_sha256':sha(ROOT/'HIGH_VALUE_SOURCE_MANIFEST.json'),
            'rule_sha256':sha(ROOT/'HIGH_VALUE_RULE.json'),'backend':backend,'workers':workers,'seconds':time.monotonic()-start,
            ...
            'native_identity':identity()if backend=='native'else None}
    return result
def main():
