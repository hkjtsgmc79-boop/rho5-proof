...
Found solve_box_fast(const EBox &nd){
 ...
 // Floating dual coefficients propose a witness; only the integer margin accepts it.
...
void emit_height(std::ostream&out,const Found&f){
...
void refine(const EBox&box,std::ostream&out){
 ...
 int safe_code=alpha_exit(box);if(safe_code>=0){out<<"A "<<safe_code<<'\n';leaves++;return;}
...
void resume_tree(const EBox&box,std::istream&in,std::ostream&out,const std::string&path=""){
 ...
 }else if(op=="A"){int code;if(!(in>>code)||!alpha_exit_proves(box,code))throw std::runtime_error("bad old alpha exit");out<<"A "<<code<<'\n';old_leaves++;
