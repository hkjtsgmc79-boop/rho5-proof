...
EXACT=('mc_exact_kernel.hpp','weighted_height.hpp','rankone_oracle.hpp','factor_graph.hpp','factor_oracle.hpp','mc_verify.cpp')
DISCOVERY=('discover_hybrid.cpp','fast_simplex.hpp','proposal_simplex.hpp')

def prepare(name):
 from verify_local_certificate import verify
 verify(json.loads((ROOT/'local_wall_certificate.json').read_text()))
 ...
 inputs=[b/n for n in EXACT+DISCOVERY+('mc_exact_model.hpp','mc_model.hpp','alpha_ports.hpp')]
...
def main():
 ...
 if current.exists():
  ...
  if set(state.get('expected_models',accepted))!=set(a.models):raise ValueError('continued campaign must keep the exact model set')
 ...
 while inputs and time.monotonic()<deadline:
  ...
  fp.write_json(current,{'status':'COMPLETE_SELECTED_ORIGINAL_ROOTS'if len(accepted)==len(a.models)and all(v['open']==0 for v in accepted.values())else'EXACT_PARTIAL_ORIGINAL_ROOTS','expected_models':a.models,'models':accepted,'workers_cap':a.workers,'ledger':'11/15'})
  print(json.dumps({'event':'original_roots_accepted','wave':index,'models':{n:{k:v[k]for k in('nodes','leaves','contradiction_leaves','alpha_safe_leaves','open','max_depth','full_factor_leaves')}for n,v in result.items()}}),flush=True)
 ...
 print(json.dumps({'status':'SELECTED_ROOTS_COMPLETE'if complete else'BUDGET_CHECKPOINTS_SAVED_NOT_A_HEIGHT_PROOF','state':str(current),'whole_R':'NOT_AUTOMATICALLY_ASSEMBLED','macro_ledger':'11/15','goal':'exact alpha, not a uniform gamma cap'}),flush=True)
