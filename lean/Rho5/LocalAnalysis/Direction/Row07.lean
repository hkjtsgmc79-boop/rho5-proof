...
theorem directionApprox_row7_identity (z : X) :
    directionApprox z 7 = (0 : ℝ) := by
  change directionBase 7 + (directionBase - preconditioner (jacobian z directionBase)) 7 = _
  rw [residualRow7_identity]
  simp only [evalTerms, residualRow7]
...
theorem directionApprox_row7_bound (z : X) (hz : z ∈ cube center radius) :
    |directionApprox z 7| ≤ (43 / 20 : ℝ) := by
  have h := centered_affine_bound (0 / 1) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ)
    (ρ := (3 / 1000 : ℝ)) (by
    intro i
    fin_cases i)
  have he : directionApprox z 7 = (0 / 1) + ∑ k, (![] : Fin 0 → ℝ) k * ((![] : Fin 0 → ℝ) k - (![] : Fin 0 → ℝ) k) := by
    rw [directionApprox_row7_identity]
