...
本文件只描述**已经编译验证过**的入口与边界；完整类型与公理见 `RELEASE_READY.json` 与
`../ACCEPTED_CORE_MAP.md`。示例源码：`src/Rho5/PhaseOne/Examples.lean`（模块 `Rho5.PhaseOne.Examples`）。
...
`src/Rho5/PhaseOne/Examples.lean` 里有 12 个真实 `theorem/example`：
