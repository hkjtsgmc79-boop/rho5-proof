...
class Simplex {
 ...
 int lastphase=0;
 int m,n;std::vector<int>B,N;std::vector<std::vector<long double>>D;
 void pivot(int r,int s){
  long double inv=1.0/D[r][s];
  for(int i=0;i<m+2;i++)if(i!=r){long double c=D[i][s]*inv;for(int j=0;j<n+2;j++)if(j!=s)D[i][j]-=D[r][j]*c;}
 ...
 bool simplex(int phase){
  ...
  for(int iter=0;iter<3000;iter++){
   ...
   int r=-1;for(int i=0;i<m;i++)if(D[i][s]>EPS){
    if(r==-1)r=i;else {
     long double a=D[i][n+1]/D[i][s],b=D[r][n+1]/D[r][s];
...
public:
 Simplex(const std::vector<std::vector<double>>&A,const std::vector<double>&b,const std::vector<double>&c):m(b.size()),n(c.size()),B(m),N(n+1),D(m+2,std::vector<long double>(n+2)){
 ...
 double solve(std::vector<double>&x){
  ...
  if(D[r][n+1]<-EPS){
   pivot(r,n);if(!simplex(1)||D[m+1][n+1]<-EPS){lastphase=1;return -std::numeric_limits<long double>::infinity();}
   if(std::abs(D[m+1][n+1])>EPS)return -std::numeric_limits<long double>::infinity();
   for(int i=0;i<m;i++)if(B[i]==-1){int s=0;for(int j=1;j<=n;j++)if(D[i][j]<D[i][s]-EPS||(std::abs(D[i][j]-D[i][s])<=EPS&&N[j]<N[s]))s=j;pivot(i,s);}
  }
  if(!simplex(2))return std::numeric_limits<long double>::infinity();
