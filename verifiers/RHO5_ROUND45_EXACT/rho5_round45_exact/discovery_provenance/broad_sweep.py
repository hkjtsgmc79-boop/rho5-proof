...
root=Path(sys.argv[1]).resolve();out=root/'broad_sweep';out.mkdir()
...
selected=[j for j in jobs if len(j['steps'])<100 and int(j['id'].split('_')[1])>=24]
print(json.dumps({'event':'broad_sweep_started','workers':36,'selected_frontiers':len(selected),'all_frontiers':len(jobs)}),flush=True)
fp.execute_wave(work,wave,selected,36,time.monotonic()+120,120)
...
print(json.dumps({'event':'broad_sweep_original_root_accepted',**{k:result[name][k] for k in ('nodes','leaves','open','max_depth')}}),flush=True)
