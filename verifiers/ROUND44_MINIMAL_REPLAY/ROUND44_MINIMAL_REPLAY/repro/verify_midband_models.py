...
import sympy as s
from build_midband_model import model


def audit(directory):
    data=json.loads((directory/'model.json').read_text())
    (vs,_,necessary,lo,hi,target),_=model(data['klo'],data['khi'],data['head_case'],data['rectangles'],data['lift'],*data['p_slice'],root_grid=None,rbranch=data.get('rbranch','all'),head_lift=data.get('head_lift',False),anchor_tail=data.get('anchor_tail','none'),stage_anchor=data.get('stage_anchor',False),fold_i=data.get('fold_i',False),moment_cuts=data.get('moment_cuts','none'))
    ...
    return dict(case=data['head_case'],K=data['klo'],J=data['khi'],rectangles=data['rectangles'],lift=data['lift'],rbranch=data.get('rbranch','all'),head_lift=data.get('head_lift',False),fold_i=data.get('fold_i',False),rows=len(names),products=len(data['pairs']),
                root_covers_analytic_necessary_root=True,exact_header_matches_model=True)


def main():
    ap=argparse.ArgumentParser();ap.add_argument('cases',nargs='+')
    ap.add_argument('--scope',choices=['full','high','low'],default='full');a=ap.parse_args()
    rows=[audit(Path(d)) for d in a.cases]
    cases={r['case'] for r in rows}
    assert cases in ({'I','II'},{'I','II','III'})
    assert len({(r['K'],r['J']) for r in rows})==1
    for case in cases:
        branches={r['rbranch'] for r in rows if r['case']==case}
        needed={'low','high'} if a.scope=='full' else {a.scope}
        assert 'all' in branches or needed<=branches,(case,branches,a.scope)
    print(json.dumps(dict(status='R44_FROZEN_MODELS_NECESSITY_AUDIT_PASS',cases=rows,
                         declared_r_scope=a.scope,
