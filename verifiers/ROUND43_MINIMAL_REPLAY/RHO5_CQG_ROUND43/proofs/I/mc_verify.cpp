...
void r43_visit_node(const EBox&box,std::istream&in){
 ...
 if(tag=="S"){
  int j;if(!(in>>j))throw std::runtime_error("invalid split value");auto[l,h]=split_box(box,j);splits++;r43_visit_node(l,in);r43_visit_node(h,in);
 }else if(tag=="C"){
...
int main(int argc,char**argv){try{
 ...
 r43_visit_node(root_box(),in);std::string unused;if(in>>unused)throw std::runtime_error("trailing unvisited records");
 ...
 std::cout<<"{\"status\":\"R43_LARGE_PIVOT_INTERVAL_EXACT_PASS\",\"nodes\":"<<nodes<<",\"splits\":"<<splits<<",\"leaves\":"<<leaves<<",\"open\":0,\"max_depth\":"<<maxdepth<<",\"max_support\":"<<maxsupport<<",\"total_support\":"<<total_support<<",\"seconds\":"<<seconds<<"}"<<std::endl;
