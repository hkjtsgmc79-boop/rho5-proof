...
theorem directionApprox_row14_identity (z : X) :
    directionApprox z 14 = (0 : ℝ) := by
  change directionBase 14 + (directionBase - preconditioner (jacobian z directionBase)) 14 = _
  rw [residualRow14_identity]
  simp only [evalTerms, residualRow14]
...
theorem directionApprox_row14_bound (z : X) (hz : z ∈ cube center radius) :
    |directionApprox z 14| ≤ (43 / 20 : ℝ) := by
  have h := centered_affine_bound (0 / 1) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ)
    (ρ := (3 / 1000 : ℝ)) (by
    intro i
    fin_cases i)
  have he : directionApprox z 14 = (0 / 1) + ∑ k, (![] : Fin 0 → ℝ) k * ((![] : Fin 0 → ℝ) k - (![] : Fin 0 → ℝ) k) := by
    rw [directionApprox_row14_identity]
