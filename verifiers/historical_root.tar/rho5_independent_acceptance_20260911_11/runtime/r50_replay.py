...
from r50_protocol import structure,binding,verify_leaf
...
def check_chunk(payload):
    ...
    for box,node in items:
        ...
        counts['enclosure:'+('R50_U40'if node['kind']=='U40'else 'V40_W40'if node['kind']=='W40'else 'V39_'+node['kind']if node['kind']in('G','V')else node.get('enclosure','original_v37'))]+=1
...
def replay(tree,workers=40,backend='native',allow_open=False):
    ...
    manifest=json.loads((ROOT/'R50_SOURCE_MANIFEST.json').read_text())
    ...
    with ProcessPoolExecutor(max_workers=workers,mp_context=multiprocessing.get_context('spawn'))as pool:
        for node in data['nodes']:
            ...
            # Isolate each expensive W40 cover so one chunk cannot serialize many covers.
            if len(batch)>=(1 if node['kind']=='W40'else 128):
    ...
    assert terminals['checked']==counts['E']+counts['C']+counts['H']+counts['V']+counts['G']+counts['W40']+counts['U40']
    ...
    result={'status':'COMPLETE_ALPHA_COVERAGE'if not counts['O']else'PARTIAL_EXACT_COVERAGE_ONLY',
            ...
            'tree_sha256':tree_hash,'model_sha256':data['model_sha256'],'source_manifest_sha256':sha(ROOT/'R50_SOURCE_MANIFEST.json'),
            'r50_binding':binding(),'backend':backend,'workers':workers,'seconds':time.monotonic()-start,
            'scope':'Fresh traversal of actual original B17 root; every old/V/G/W40 terminal checked on its actual dyadic box; all original siblings retained',
            'old_leaf_backend':backend,'new_leaf_backend':'V39 original Fraction reference and V40 original Fraction reference; not independent new implementations'}
    return result
def main():
