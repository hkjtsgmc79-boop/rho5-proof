...
theorem directionApprox_row12_identity (z : X) :
    directionApprox z 12 = (0 : ℝ) := by
  change directionBase 12 + (directionBase - preconditioner (jacobian z directionBase)) 12 = _
  rw [residualRow12_identity]
  simp only [evalTerms, residualRow12]
...
theorem directionApprox_row12_bound (z : X) (hz : z ∈ cube center radius) :
    |directionApprox z 12| ≤ (43 / 20 : ℝ) := by
  have h := centered_affine_bound (0 / 1) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ)
    (ρ := (3 / 1000 : ℝ)) (by
    intro i
    fin_cases i)
  have he : directionApprox z 12 = (0 / 1) + ∑ k, (![] : Fin 0 → ℝ) k * ((![] : Fin 0 → ℝ) k - (![] : Fin 0 → ℝ) k) := by
    rw [directionApprox_row12_identity]
