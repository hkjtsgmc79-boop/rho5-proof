...
public:
 Simplex(const std::vector<std::vector<double>>&A,const std::vector<double>&b,const std::vector<double>&c):m(b.size()),n(c.size()),B(m),N(n+1),D(m+2,std::vector<double>(n+2)){
 ...
 double solve(std::vector<double>&x){
  ...
  x.assign(n,0.);for(int i=0;i<m;i++)if(B[i]<n)x[B[i]]=D[i][n+1];return D[m][n+1];
 }
};
