...
theorem directionApprox_row9_identity (z : X) :
    directionApprox z 9 = (0 : ℝ) := by
  change directionBase 9 + (directionBase - preconditioner (jacobian z directionBase)) 9 = _
  rw [residualRow9_identity]
  simp only [evalTerms, residualRow9]
...
theorem directionApprox_row9_bound (z : X) (hz : z ∈ cube center radius) :
    |directionApprox z 9| ≤ (43 / 20 : ℝ) := by
  have h := centered_affine_bound (0 / 1) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ)
    (ρ := (3 / 1000 : ℝ)) (by
    intro i
    fin_cases i)
  have he : directionApprox z 9 = (0 / 1) + ∑ k, (![] : Fin 0 → ℝ) k * ((![] : Fin 0 → ℝ) k - (![] : Fin 0 → ℝ) k) := by
    rw [directionApprox_row9_identity]
