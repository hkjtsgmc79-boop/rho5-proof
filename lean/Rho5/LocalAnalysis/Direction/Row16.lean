...
theorem directionApprox_row16_identity (z : X) :
    directionApprox z 16 = (0 : ℝ) := by
  change directionBase 16 + (directionBase - preconditioner (jacobian z directionBase)) 16 = _
  rw [residualRow16_identity]
  simp only [evalTerms, residualRow16]
...
theorem directionApprox_row16_bound (z : X) (hz : z ∈ cube center radius) :
    |directionApprox z 16| ≤ (43 / 20 : ℝ) := by
  have h := centered_affine_bound (0 / 1) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ) (![] : Fin 0 → ℝ)
    (ρ := (3 / 1000 : ℝ)) (by
    intro i
    fin_cases i)
  have he : directionApprox z 16 = (0 / 1) + ∑ k, (![] : Fin 0 → ℝ) k * ((![] : Fin 0 → ℝ) k - (![] : Fin 0 → ℝ) k) := by
    rw [directionApprox_row16_identity]
